...
router.post('/',async(req,res)=>{
    ...
    const response = await drive.files.create({
        ...
        await socket.emit('pdftoimage',{response:response.data,socket:req.headers['d-custom'],name})
        let tries = true
        await socket.on('processedpdftoimage',async({data,socket1})=>{
            if (socket1 == req.headers['d-custom'] &&tries) {
                tries = false
